...
parser.add_argument('--hidden-units', type=str, default="16,16",
...
parser.add_argument('--file-dir', type=str, default='/Users/crescendo/Projects/DouyinDataAnalyse/data/', help="Input file directory")
...
N = len(influence_dataset) # 图的数目
n_classes = 2 # label数量
class_weight = influence_dataset.get_class_weight() \
        if args.class_weight_balanced else torch.ones(n_classes)  # 各label的占比
logger.info("class_weight=%.2f:%.2f", class_weight[0], class_weight[1])

feature_dim = influence_dataset.get_feature_dimension()
n_units = [feature_dim] + [int(x) for x in args.hidden_units.strip().split(",")] + [n_classes] # unit数量
logger.info("feature dimension=%d", feature_dim)
logger.info("number of classes=%d", n_classes)
logger.info('n_unit={}'.format(n_units))

train_start,  valid_start, test_start = \
        0, int(N * args.train_ratio / 100), int(N * (args.train_ratio + args.valid_ratio) / 100) # 三分
...
if args.model == "gcn":
    ...
    logger.info('n_heads={}'.format(n_heads))
